...
logflux_OIII5008, dlogflux_OIII5008 = load_lineratio(region='S8')

# Load cloudy result
Hden = np.arange(-2, 2.6, 0.1)  # log # 0.25
metal = np.linspace(-1.5, 0.5, 11, dtype='f2')
alpha = np.linspace(-1.2, 0, 13, dtype='f2')
output, ind = format_cloudy(filename=[metal, alpha], path='/Users/lzq/Dropbox/Data/CGM/cloudy/trial5/')
...
ndim, nwalkers = 3, 40
p0 = np.array([1.8, -0.8, -0.3]) + 0.1 * np.random.randn(nwalkers, ndim)
...
figure.savefig('/Users/lzq/Dropbox/Data/CGM_plots/Cloudy_test_mcmc_S8_t5.pdf', bbox_inches='tight')