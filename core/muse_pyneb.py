...
den_array_den = 10 ** np.linspace(1, 4, 50)
...
# [O II] vs temperature
tem_array_tem = 10 ** np.linspace(4.09, 4.11, 1000)
...
for i in range(len(den_array_tem)):
    axarr[1].plot(np.log10(tem_array_tem), np.log10(OII3729_tem/OII3727_tem)[:, i], '-', label='n=' +
                                                                                            str(den_array_tem[i]))
axarr[1].set_xlabel(r"$\mathrm{Temperature \, [K]}$", size=15)
...
fig, axarr = plt.subplots(2, 1, figsize=(5, 10), dpi=300)
tem_array_den = np.array([8000, 10000, 20000])
den_array_den = 10 ** np.linspace(2.99, 3.02, 50)
...
for i in range(len(den_array_tem)):
    axarr[1].plot(np.log10(tem_array_tem), np.log10(OIII4363_tem/OIII5007_tem)[:, i], '-', label='n=' +
                                                                                            str(den_array_tem[i]))
axarr[1].set_xlabel(r"$\mathrm{Temperature \, [K]}$", size=15)
...
# def log_prob(x):
#     tem, den = x[0], x[1]
#     return - 0.5 * ((f(tem, den) - true_line_ratio) / 100) ** 2
#
# ndim, nwalkers = 2, 40
