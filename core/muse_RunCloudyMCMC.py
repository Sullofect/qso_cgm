...
def log_prob(x, bnds, line_param, mode, f_NeV3346, f_OII, f_NeIII3869, f_Hdel, f_Hgam, f_OIII4364,
    ...
    if mode == 'power_law':
        ...
        if logden < bnds[0, 0] or logden > bnds[0, 1]:
            ...
            return -np.inf
        elif alpha_x < bnds[4, 0] or alpha_x > bnds[4, 1]:
            return -np.inf
        else:
...
def RunCloudyMCMC(den_array=den_default, Z_array=Z_default, T_array=None, alpha_array=alpha_default,
    ...
    data_x1 = [0, 5]
    data_x2 = [E_OII / E_Hbeta, E_OIII4364 / E_Hbeta - 0.5, E_OIII5008 / E_Hbeta - 0.5,
               E_NeIII3869 / E_Hbeta + 0.2, E_HeII4687 / E_Hbeta, E_NeV3346 / E_Hbeta - 2]
    ...
    model_y2 = np.array([f_OII(model), f_OIII4364(model), f_OIII5008(model),
                         f_NeIII3869(model), f_HeII4687(model), f_NeV3346(model)])
    ...
    # Hist comparison
    if compare_hist:
        samples_OII = PlotGasSpectra(region, figname='spectra_gas_' + region, deredden=False,
        ...
        bins_cloudy = np.arange(sample_min//10 * 10, sample_max//10 * 10, 10)

        ax[2].hist(10 ** samples[:, 0], bins=bins_cloudy, density=True, facecolor='brown', histtype='stepfilled',
                   alpha=0.5, label=r'$\rm Cloudy$')
        ax[2].hist(samples_OII[:, 0], bins=bins_cloudy, density=True, facecolor='blue', histtype='stepfilled',
                   alpha=0.5, label=r'$\rm [O \, II]$')
        ax[2].minorticks_on()
        ax[2].set_xlabel(r'$\mathrm{n_{H}[cm^{-3}]}$', size=20)
...
#               bnds=S6_bnds, line_param=S6_param, deredden=False, mode='AGN_nouv')
