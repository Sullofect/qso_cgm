...
for i in range(np.shape(cube_OIII)[0]):
    fig = plt.figure(figsize=(8, 8), dpi=300)
    wave_i = cube_OIII.wave.coord()[i]
    sub_cube = cube_OIII.select_lambda(wave_i)
    ...
    gc.tick_labels.hide()
    gc.axis_labels.hide()
    gc.add_label(0.95, 0.97, r'[O III]', size=15, relative=True)
    ...
    fig.savefig('/Users/lzq/Dropbox/Data/CGM_plots/NB_movie/image_OIII_' + str("{0:.0f}".format(wave_i))
...
# os.system('ffmpeg -f gif -i ~/dropbox/Data/CGM_plots/NB_movie/OII_movie.gif '
#           '~/dropbox/Data/CGM_plots/NB_movie/OII_movie.mp4')
