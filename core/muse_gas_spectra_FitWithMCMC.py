...
def model_MCMC(wave_vac, z, dz_wing, sigma_kms, sigma_kms_wing, flux_OII, flux_OII_wing, flux_OIII5008,
               flux_OIII5008_wing, r_OII3729_3727, r_OII3729_3727_wing, a_OII, b_OII, a_OIII5008, b_OIII5008):
    # Strong lines
    m_OII = model_OII(wave_vac[8], z, sigma_kms, flux_OII, r_OII3729_3727, a_OII, b_OII)
    m_OIII5008 = model_OIII5008(wave_vac[11], z, sigma_kms, flux_OIII5008, a_OIII5008, b_OIII5008)
    ...
    m_OIII5008_wing = model_OIII5008(wave_vac[11], z + dz_wing, sigma_kms_wing, flux_OIII5008_wing,
                                     a_OIII5008, b_OIII5008)
    return np.hstack((m_OII + m_OII_wing, m_OIII5008 + m_OIII5008_wing))
...
def log_prob(x, wave_vac, flux, dflux, a_OII, b_OII, a_OIII5008, b_OIII5008):
    ...
    if z < 0.62 or z > 0.63:
        ...
        model = model_MCMC(wave_vac, z, dz_wing, sigma_kms, sigma_kms_wing, flux_OII, flux_OII_wing, flux_OIII5008,
                           flux_OIII5008_wing, r_OII3729_3727, r_OII3729_3727_wing, a_OII, b_OII, a_OIII5008,
                           b_OIII5008)
        L = - 0.5 * np.nansum(((model - flux) / dflux) ** 2)
        # print(L)
        return L
...
def PlotGasSpectra(region=None, figname='spectra_gas_1', deredden=True, save_table=False, save_figure=True,
    ...
    for i in range(len(ra_array)):
        ...
        flux_MCMC = np.hstack((flux_OII_i, flux_OIII5008_i))
        flux_err_MCMC = np.hstack((flux_OII_err_i, flux_OIII5008_err_i))

        if deredden:
            filename = '/Users/lzq/Dropbox/Data/CGM_plots/spectra_gas/' + region + '_OII_dered.h5'
            figname_MCMC = '/Users/lzq/Dropbox/Data/CGM_plots/' + figname + '_OII_MCMC_dered.pdf'
        else:
            filename = '/Users/lzq/Dropbox/Data/CGM_plots/spectra_gas/' + region + '_OII.h5'
            figname_MCMC = '/Users/lzq/Dropbox/Data/CGM_plots/' + figname + '_OII_MCMC.pdf'
        ...
        figure = corner.corner(samples, labels=[r"$\mathrm{z}$", r"$\mathrm{dz\_wing}$",
                                                r"$\mathrm{\sigma \, (km \, s^{-1})}$",
                                                r"$\mathrm{\sigma\_wing \, (km \, s^{-1})}$",
        ...
        for j in range(3):
            ...
            ind_2 = len(wave_vac_all[8]) + len(wave_vac_all[11])
            axarr_0_strong.plot(wave_vac_all[8], model_MCMC_j[:ind_1], '-', lw=0.5, zorder=100)
            axarr_1_strong.plot(wave_vac_all[11], model_MCMC_j[ind_1:ind_2], '-', lw=1, zorder=100)
            #
