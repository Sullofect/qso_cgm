...
ra_ste = ra_final[select_gal]
dec_ste = dec_final[select_gal]
...
ra_center_blue, dec_center_blue = np.sum(ra_final * P_blue) / norm_blue, \
                                  np.sum(dec_final * P_blue) / norm_blue
...
ra_center_red, dec_center_red = np.sum(ra_final * P_red) / norm_red, \
                                np.sum(dec_final * P_red) / norm_red
...
# Markers
gc.show_markers(ra_center_all, dec_center_all, marker=(6, 2, 0), c='k', s=30)
