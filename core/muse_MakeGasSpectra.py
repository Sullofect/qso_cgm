...
def model_all_S3S4(wave_vac, z, dz_wing, sigma_kms, sigma_kms_wing, flux_NeV3346, flux_NeV3346_wing, flux_NeIII3869,
...
class MakeGasSpectra:
    def __init__(self, region=None, figname='spectra_gas_1', deredden=True, save_table=False, save_figure=True):
        self.region = region
        ...
        # Weak lines
        cube = Cube(path_cube)
        self.cube_NeV3346 = cube.select_lambda(5380, 5500)
        self.cube_NeIII3869 = cube.select_lambda(6250, 6320)
        self.cube_HeI3889 = cube.select_lambda(6310, 6350)
        self.cube_Heps = cube.select_lambda(6430, 6490)
        self.cube_Hdel = cube.select_lambda(6640, 6710)
        self.cube_Hgam = cube.select_lambda(7020, 7090)
        self.cube_OIII4364 = cube.select_lambda(7085, 7125)
        self.cube_HeII4687 = cube.select_lambda(7595, 7660)

        wave_NeV3346_vac = pyasl.airtovac2(self.cube_NeV3346.wave.coord())
        wave_NeIII3869_vac = pyasl.airtovac2(self.cube_NeIII3869.wave.coord())
        wave_HeI3889_vac = pyasl.airtovac2(self.cube_HeI3889.wave.coord())
        wave_Heps_vac = pyasl.airtovac2(self.cube_Heps.wave.coord())
        wave_Hdel_vac = pyasl.airtovac2(self.cube_Hdel.wave.coord())
        wave_Hgam_vac = pyasl.airtovac2(self.cube_Hgam.wave.coord())
        wave_OIII4364_vac = pyasl.airtovac2(self.cube_OIII4364.wave.coord())
        wave_HeII4687_vac = pyasl.airtovac2(self.cube_HeII4687.wave.coord())
        ...
        # Strong lines
        self.wave_vac_strong_stack = np.hstack((wave_OII_vac, wave_Hbeta_vac, wave_bet_vac,
                                                wave_OIII4960_vac, wave_OIII5008_vac))
        self.idx_strong = (len(self.wave_vac_strong_stack) - len(wave_bet_vac)) * 3

        # All lines
        self.wave_vac_all = np.array([wave_NeV3346_vac, wave_NeIII3869_vac, wave_HeI3889_vac, wave_Heps_vac, wave_Hdel_vac,
        ...
        self.wave_vac_all_plot = expand_wave(self.wave_vac_all, stack=False)
        # wave_vac_all_stack = np.hstack((wave_NeV3346_vac, wave_NeIII3869_vac, wave_HeI3889_vac, wave_Heps_vac, wave_Hdel_vac,
        #                                  wave_Hgam_vac, wave_OIII4364_vac, wave_HeII4687_vac, wave_OII_vac, wave_Hbeta_vac,
        #                                  wave_OIII4960_vac, wave_OIII5008_vac))
        self.wave_vac_all_stack = expand_wave(self.wave_vac_all)
        self.idx_all = len(self.wave_vac_all_stack)
        self.idx_weak = self.idx_all - self.idx_strong
        ...
        self.ra_array = np.loadtxt(path_region, usecols=[0, 1, 2], delimiter=',')[:, 0]
        self.dec_array = np.loadtxt(path_region, usecols=[0, 1, 2], delimiter=',')[:, 1]
        self.radius_array = np.loadtxt(path_region, usecols=[0, 1, 2], delimiter=',')[:, 2]
        self.text_array = np.loadtxt(path_region, dtype=str, usecols=[3], delimiter=',')

        region_mask = np.in1d(self.text_array, self.region)
        ra_array, dec_array, radius_array, text_array = self.ra_array[region_mask], self.dec_array[region_mask], \
                                                        self.radius_array[region_mask], self.text_array[region_mask]

        self.parameters_all = lmfit.Parameters()

        if region == 'S3' or region == 'S4':
            flux_info = np.zeros((len(self.region), 60))
            self.model_all = model_all_S3S4
            self.parameters_all.add_many(('z', redshift_guess, True, 0.62, 0.64, None),
                                    ...
                                    ('flux_OII_wing', 0.005, True, 0, 5, None),
            ...
            flux_info = np.zeros((len(self.region), 30))
        ...
        for i in range(len(ra_array)):
            # Unit in arcsec
            spe_NeV3346_i = self.cube_NeV3346.aperture((dec_array[i], ra_array[i]), radius_array[i], is_sum=True)
            spe_NeIII3869_i = self.cube_NeIII3869.aperture((dec_array[i], ra_array[i]), radius_array[i], is_sum=True)
            spe_HeI3889_i = self.cube_HeI3889.aperture((dec_array[i], ra_array[i]), radius_array[i], is_sum=True)
            spe_Heps_i = self.cube_Heps.aperture((dec_array[i], ra_array[i]), radius_array[i], is_sum=True)
            spe_Hdel_i = self.cube_Hdel.aperture((dec_array[i], ra_array[i]), radius_array[i], is_sum=True)
            spe_Hgam_i = self.cube_Hgam.aperture((dec_array[i], ra_array[i]), radius_array[i], is_sum=True)
            spe_OIII4364_i = self.cube_OIII4364.aperture((dec_array[i], ra_array[i]), radius_array[i], is_sum=True)
            spe_HeII4687_i = self.cube_HeII4687.aperture((dec_array[i], ra_array[i]), radius_array[i], is_sum=True)
            spe_OII_i = self.cube_OII.aperture((dec_array[i], ra_array[i]), radius_array[i], is_sum=True)
            spe_Hbeta_i = self.ube_Hbeta.aperture((dec_array[i], ra_array[i]), radius_array[i], is_sum=True)
            spe_bet_i = self.cube_bet.aperture((dec_array[i], ra_array[i]), radius_array[i], is_sum=True)
            spe_OIII4960_i = self.cube_OIII4960.aperture((dec_array[i], ra_array[i]), radius_array[i], is_sum=True)
            spe_OIII5008_i = self.cube_OIII5008.aperture((dec_array[i], ra_array[i]), radius_array[i], is_sum=True)
            ...
            if deredden:
                A_v = 0.087
                factor_extinction = extinction_ndarray(self.wave_vac_all, A_v)
            ...
            flux_all = np.hstack((flux_NeV3346_i, flux_NeIII3869_i, flux_HeI3889_i, flux_Heps_i, flux_Hdel_i,
                                  flux_Hgam_i, flux_OIII4364_i, flux_HeII4687_i, flux_OII_i, flux_Hbeta_i,
                                  flux_OIII4960_i, flux_OIII5008_i))
            ...
            result_all = spec_model_all.fit(data=flux_all, wave_vac=self.wave_vac_all, params=self.parameters_all,
            ...
            if region == 'S3' or region == 'S4':
                ...
                sigma_wing, dsigma_wing = result_all.best_values['sigma_kms_wing'], result_all.params[
                    'sigma_kms_wing'].stderr
                ...
                # Weak lines
                flux_NeV3346, dflux_NeV3346 = result_all.best_values['flux_NeV3346'], result_all.params[
                    'flux_NeV3346'].stderr
                ...
                flux_HeI3889, dflux_HeI3889 = result_all.best_values['flux_HeI3889'], result_all.params[
                    'flux_HeI3889'].stderr
                ...
                flux_H8_wing, dflux_H8_wing = result_all.best_values['flux_H8_wing'], result_all.params[
                    'flux_H8_wing'].stderr
                ...
                a_NeIII3869, da_NeIII3869 = result_all.best_values['a_NeIII3869'], result_all.params[
                    'a_NeIII3869'].stderr
                b_NeIII3869, db_NeIII3869 = result_all.best_values['b_NeIII3869'], result_all.params[
                    'b_NeIII3869'].stderr
                ...
                a_NeIII3968, da_NeIII3968 = result_all.best_values['a_NeIII3968'], result_all.params[
                    'a_NeIII3968'].stderr
                b_NeIII3968, db_NeIII3968 = result_all.best_values['b_NeIII3968'], result_all.params[
                    'b_NeIII3968'].stderr
                ...
                flux_info[i, :] = np.array([z, dz_wing, sigma, sigma_wing, flux_NeV3346, flux_NeV3346_wing, flux_NeIII3869,
                                            ...
                                            dflux_Hgam, dflux_Hgam_wing, dflux_OIII4364, dflux_OIII4364_wing,
                                            dflux_HeII4687,
                                            dflux_HeII4687_wing, dflux_OII, dflux_OII_wing, dr_OII,
                                            dr_OII_wing, dflux_Hbeta, dflux_Hbeta_wing, dflux_OIII5008,
                                            dflux_OIII5008_wing])
                ...
                # Save the fitted result
                flux_info[i, :] = np.array([z, sigma, flux_NeV3346, flux_NeIII3869, flux_HeI3889, flux_H8, flux_NeIII3968,
                                            flux_Heps, flux_Hdel, flux_Hgam, flux_OIII4364, flux_HeII4687, flux_OII, r_OII,
                                            flux_Hbeta,
        ...
        t = Table(flux_info, names=('z', 'sigma', 'flux_NeV3346', 'flux_NeIII3869', 'flux_HeI3889', 'flux_H8',
                                    ...
                                    'flux_HeII4687', 'flux_OII', 'r_OII', 'flux_Hbeta', 'flux_OIII5008',
                                    'dflux_NeV3346',
    ...
    if save_table is True:
        if deredden:
            ...
            t.write('/Users/lzq/Dropbox/Data/CGM/RegionLinesRatio/RegionLinesRatio.fits', format='fits',
                    overwrite=True)

    def FitGasSpectra(self, figname='spectra_gas_1', deredden=True, save_table=False, save_figure=True):
        region_mask = np.in1d(self.text_array, self.region)
        ra_array, dec_array, radius_array, text_array = self.ra_array[region_mask], self.dec_array[region_mask], \
                                                        self.radius_array[region_mask], self.text_array[region_mask]
        ...
        for i in range(len(ra_array)):
            ...
            flux_all = np.hstack((flux_NeV3346_i, flux_NeIII3869_i, flux_HeI3889_i, flux_Heps_i, flux_Hdel_i, flux_Hgam_i,
                                  flux_OIII4364_i, flux_HeII4687_i, flux_OII_i, flux_Hbeta_i, flux_OIII4960_i, flux_OIII5008_i))
            flux_err_all = np.hstack((flux_NeV3346_err_i, flux_NeIII3869_err_i, flux_HeI3889_err_i, flux_Heps_err_i,
                                      flux_Hdel_err_i, flux_Hgam_err_i, flux_OIII4364_err_i, flux_HeII4687_err_i,
                                      flux_OII_err_i, flux_Hbeta_err_i, flux_OIII4960_err_i, flux_OIII5008_err_i))
            ...
            # Save the fitted result
            flux_info[i, :] = np.array([z, sigma, flux_NeV3346, flux_NeIII3869, flux_HeI3889, flux_H8, flux_NeIII3968,
                                        flux_Heps, flux_Hdel, flux_Hgam, flux_OIII4364, flux_HeII4687, flux_OII, r_OII, flux_Hbeta,
                 flux_OIII5008, dflux_NeV3346, dflux_NeIII3869, dflux_HeI3889, dflux_H8,
                 dflux_NeIII3968, dflux_Heps, dflux_Hdel, dflux_Hgam, dflux_OIII4364,
                 dflux_HeII4687, dflux_OII, dr_OII, dflux_Hbeta, dflux_OIII5008])
                ...
                line_model_all = model_all(wave_vac_all_plot, z, dz_wing, sigma, sigma_wing, flux_NeV3346,
                                           ...
                                           flux_Hgam_wing,
                                           flux_OIII4364, flux_OIII4364_wing, flux_HeII4687, flux_HeII4687_wing,
                                           flux_OII,
                                           flux_OII_wing, flux_Hbeta, flux_Hbeta_wing, flux_OIII5008,
                                           flux_OIII5008_wing,
                                           r_OII, r_OII_wing, a_NeV3346, b_NeV3346, a_NeIII3869, b_NeIII3869,
                                           a_HeI3889, b_HeI3889, a_NeIII3968, b_NeIII3968, a_Hdel, b_Hdel, a_Hgam,
                                           b_Hgam,
                                           a_OIII4364, b_OIII4364, a_HeII4687, b_HeII4687, a_OII, b_OII, a_Hbeta,
                                           b_Hbeta,
                                           a_OIII4960, b_OIII4960, a_OIII5008, b_OIII5008)

            else:
                line_model_all = model_all(wave_vac_all_plot, z, sigma, flux_NeV3346, flux_NeIII3869, flux_HeI3889,
                                           flux_H8,
                                           flux_NeIII3968, flux_Heps, flux_Hdel, flux_Hgam, flux_OIII4364,
                                           flux_HeII4687,
                                           ...
                                           b_HeII4687,
                                           a_OII, b_OII, a_Hbeta, b_Hbeta, a_OIII4960, b_OIII4960, a_OIII5008,
                                           b_OIII5008)
    ...
    # Def Plot function
    def PlotGasSpectra(self, figname='spectra_gas_1', deredden=True, save_table=False, save_figure=True):
        region_mask = np.in1d(self.text_array, self.region)
        ra_array, dec_array, radius_array, text_array = self.ra_array[region_mask], self.dec_array[region_mask], \
                                                        self.radius_array[region_mask], self.text_array[region_mask]
        ...
        for i in range(len(ra_array)):
            ...
            # Load the data
            flux_NeV3346_i, flux_NeV3346_err_i = spe_NeV3346_i.data * 1e-3, np.sqrt(spe_NeV3346_i.var) * 1e-3
            flux_NeIII3869_i, flux_NeIII3869_err_i = spe_NeIII3869_i.data * 1e-3, np.sqrt(
                spe_NeIII3869_i.var) * 1e-3
            flux_HeI3889_i, flux_HeI3889_err_i = spe_HeI3889_i.data * 1e-3, np.sqrt(spe_HeI3889_i.var) * 1e-3
            flux_Heps_i, flux_Heps_err_i = spe_Heps_i.data * 1e-3, np.sqrt(spe_Heps_i.var) * 1e-3
            flux_Hdel_i, flux_Hdel_err_i = spe_Hdel_i.data * 1e-3, np.sqrt(spe_Hdel_i.var) * 1e-3
            flux_Hgam_i, flux_Hgam_err_i = spe_Hgam_i.data * 1e-3, np.sqrt(spe_Hgam_i.var) * 1e-3
            ...
            flux_all = np.hstack(
                (flux_NeV3346_i, flux_NeIII3869_i, flux_HeI3889_i, flux_Heps_i, flux_Hdel_i, flux_Hgam_i,
            ...
            # Weak lines
            flux_NeV3346, dflux_NeV3346 = result_all.best_values['flux_NeV3346'], \
                                          result_all.params['flux_NeV3346'].stderr
            flux_NeIII3869, dflux_NeIII3869 = result_all.best_values['flux_NeIII3869'], \
                                              result_all.params['flux_NeIII3869'].stderr
            flux_HeI3889, dflux_HeI3889 = result_all.best_values['flux_HeI3889'], result_all.params[
                'flux_HeI3889'].stderr
            ...
            # Save the fitted result
            flux_info[i, :] = np.array(
                [z, sigma, flux_NeV3346, flux_NeIII3869, flux_HeI3889, flux_H8, flux_NeIII3968, flux_Heps,
            ...
            if text_array[i] == 'S3' or text_array[i] == 'S4':
                ...
                flux_NeIII3968_wing, flux_Heps_wing = lineRatioS3S4['flux_NeIII3968_wing'], lineRatioS3S4[
                    'flux_Heps_wing']
                ...
                flux_Hbeta_wing, flux_OIII5008_wing = lineRatioS3S4['flux_Hbeta_wing'], lineRatioS3S4[
                    'flux_OIII5008_wing']
            ...
            axarr_i_weak[0].plot(wave_NeV3346_vac, flux_NeV3346_err_i, color='lightgrey', drawstyle='steps-mid',
                                 lw=1)
            ...
            axarr_i_weak[4].plot(wave_OIII4364_vac, flux_OIII4364_err_i, color='lightgrey', drawstyle='steps-mid',
                                 lw=1)
            ...
            axarr_i_weak[5].plot(wave_HeII4687_vac, flux_HeII4687_err_i, color='lightgrey', drawstyle='steps-mid',
                                 lw=1)
            ...
            axarr_0_weak[0].annotate(text=r'$\mathrm{[Ne \, V]}$', xy=(0.1, 0.65), xycoords='axes fraction',
                                     size=15)
            ...
            axarr_0_weak[4].annotate(text=r'$\mathrm{[O \, III]}$', xy=(0.74, 0.65), xycoords='axes fraction',
                                     size=15)
            ...
            axarr_i_weak[0].tick_params(axis='both', which='major', direction='in', top='on', bottom='on',
                                        left='on',
                                        right=False, labelsize=20, size=5)
            axarr_i_weak[0].tick_params(axis='both', which='minor', direction='in', top='on', bottom='on',
                                        left='on',
                                        right=False, size=3)
            axarr_i_weak[0].tick_params(axis='y', which='both', right=False, labelright=False)
            axarr_i_weak[5].tick_params(axis='both', which='major', direction='in', top='on', bottom='on',
                                        left=False,
                                        right='on', labelsize=20, size=5)
            axarr_i_weak[5].tick_params(axis='both', which='minor', direction='in', top='on', bottom='on',
                                        left=False,
                                        right='on', size=3)
            axarr_i_weak[5].tick_params(axis='y', which='both', left=False, labelleft=False)
            for j in [1, 2, 3, 4]:
                axarr_i_weak[j].tick_params(axis='both', which='major', direction='in', top='on', bottom='on',
                                            left=False,
                                            right=False, labelsize=20, size=5)
                axarr_i_weak[j].tick_params(axis='both', which='minor', direction='in', top='on', bottom='on',
                                            left=False,
                                            right=False, size=3)
            ...
            if i == 0:
                axarr_0_strong.annotate(text=r'$\mathrm{[O \, II]}$', xy=(0.6, 0.65), xycoords='axes fraction',
                                        size=20)
                axarr_1_strong.annotate(text=r'$\mathrm{H\beta}$', xy=(0.1, 0.65), xycoords='axes fraction',
                                        size=20)
                axarr_1_strong.annotate(text=r'$\mathrm{[O \, III]}$', xy=(0.45, 0.65), xycoords='axes fraction',
                                        size=20)
                axarr_1_strong.annotate(text=r'$\mathrm{[O \, III]}$', xy=(0.7, 0.65), xycoords='axes fraction',
                                        size=20)
            ...
            axarr_1_strong.tick_params(axis='both', which='major', direction='in', top='on', bottom='on',
                                       left=False,
                                       right='on', labelsize=20, size=5)
            axarr_1_strong.tick_params(axis='both', which='minor', direction='in', top='on', bottom='on',
                                       left=False,
                                       right='on', size=3)
        ...
        if save_figure:
            if deredden:
                fig_weak.savefig('/Users/lzq/Dropbox/Data/CGM_plots/' + figname + '_weak_dered.png',
                                 bbox_inches='tight')
                fig_strong.savefig('/Users/lzq/Dropbox/Data/CGM_plots/' + figname + '_strong_dered.png',
                                   bbox_inches='tight')
            else:
                fig_weak.savefig('/Users/lzq/Dropbox/Data/CGM_plots/' + figname + '_weak.png', bbox_inches='tight')
                fig_strong.savefig('/Users/lzq/Dropbox/Data/CGM_plots/' + figname + '_strong.png',
                                   bbox_inches='tight')
